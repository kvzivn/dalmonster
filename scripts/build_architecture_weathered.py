...
material('Cosy_HeroRoom',(.145,.095,.044),.97,0,.16)
material('Cosy_WindowFilm',(.15,.22,.24),.27,.24)
f=mats['Cosy_WindowFilm'];f.node_tree.nodes['Principled BSDF'].inputs['Alpha'].default_value=.13;f.surface_render_method='DITHERED'
for mat,file in [('Cosy_DampDecal','facade-finish-base-damp.png'),('Cosy_RunoffDecal','facade-finish-sill-runoff.png')]:
 material(mat,(1,1,1),1);m=mats[mat];tx=m.node_tree.nodes.new('ShaderNodeTexImage');tx.image=bpy.data.images.load(str(ROOT/'public/assets'/file),check_existing=True);tx.image.pack();bs=m.node_tree.nodes['Principled BSDF'];m.node_tree.links.new(tx.outputs['Color'],bs.inputs['Base Color'])
 mult=m.node_tree.nodes.new('ShaderNodeMath');mult.operation='MULTIPLY';mult.inputs[1].default_value=.42 if 'Damp' in mat else .28;m.node_tree.links.new(tx.outputs['Alpha'],mult.inputs[0]);m.node_tree.links.new(mult.outputs[0],bs.inputs['Alpha']);m.surface_render_method='DITHERED'
...
for name,factor in [('Cosy_CurtainCream',(.5,.43,.32,1)),('Cosy_CurtainBlue',(.105,.175,.205,1))]:
 m=mats[name];tx=m.node_tree.nodes.new('ShaderNodeTexImage');tx.image=bpy.data.images.load(str(ROOT/'public/assets/facade-finish-curtain-weave.png'),check_existing=True);tx.image.pack();m.node_tree.links.new(tx.outputs['Color'],m.node_tree.nodes['Principled BSDF'].inputs['Base Color'])
 texture_factors[name]=factor
...
class Face:
 ...
 def window(self,x,z,w,h,seed,arched=False,door=False):
  ...
  hero=(not door and self.g=='Building_North' and self.o.x < -17 and self.o.y>15 and 4.2<z<9.0)
  # Selected close rooms sit60cm behind the glazing rather than at the reveal face.
  ...
  else:self.box('Cosy_HeroRoom' if hero and lit else room,x,.85 if hero else .42,z,w,.035,h)
  if hero:
   self.box('Cosy_DarkWood',x,.48,top-.035,w,.78,.055)
   self.box('Cosy_DarkWood',x,.62,sill+.38,w*.56,.025,.055)
   # A tiny plant/vase silhouette against a dark actual room.
   self.box('Cosy_DarkWood',x+w*.2,.61,sill+.52,.11,.065,.25)
   self.panel('Cosy_WindowFilm',[(x-w/2,sill),(x+w/2,sill),(x+w/2,top),(x-w/2,top)],.18)
  for side in [-1,1]:self.box('Cosy_StoneBase',x+side*(w/2+.025),.40 if hero else .20,z,.055,.82 if hero else .43,h)
  ...
  if not door:
   for side in [-1,1]:
    width=w*(.34 if side<0 else .16) if hero else w*rr.uniform(.14,.36);center=x+side*(w*.47-width*.5);vs=[]
    for j in range(13):
     u=center+(j/12-.5)*width;d=.24+.027*math.cos(j*math.pi/2)
     if hero:
      # Three broad gathered folds, a hanging hem and asymmetrical fabric opening.
      dd=.30+.048*math.cos(j*math.pi/2);bottom=center+(u-center)*.86+.021*math.sin(j*.7)
      vs.extend([self.P(bottom,dd,sill+.11+.042*math.cos(j*.55)),self.P(u,dd+.018,top-.04)])
     else:vs.extend([self.P(u,d,sill+.08+.018*math.sin(j)),self.P(u,d,top-.04)])
 ...
 def wallbay(self,w,z0,z1,winz,winw,winh,mat,seed,arch=False,door=False):
  ...
  if z0<.2:decal(self,0,.19,w,.48,'Cosy_DampDecal')
  if z0>=3 and sum(map(ord,str(seed)))%3==0:decal(self,0,winz-winh/2-.44,winw+.21,.34,'Cosy_RunoffDecal')

def decal(f,x,z,w,h,mat):
 verts=[f.P(x-w/2,-.008,z),f.P(x+w/2,-.008,z),f.P(x+w/2,-.008,z+h),f.P(x-w/2,-.008,z+h)]
 mesh(mat,verts,[(0,1,2,3)],f.g,uv=[(0,1),(1,1),(1,0),(0,0)])
...
for label,r,st,en,upper in [('North',27,39,134,'Cosy_LimeIvory'),('West',29,151,218,'Cosy_LimeOchre')]:
 ...
 for a,isend in [(a0,False),(a1,True)]:
  ...
  decal(rf,0,.22,8.0,.49,'Cosy_DampDecal')
  for j in range(4):
   for side in [-1,1]:
...
# Fine per-piece patina is baked into vertex color, not per-frame noise.
for ob in bpy.context.scene.objects:
 if ob.type!='MESH':continue
 mat=ob.data.materials[0].name;me=ob.data;colors=me.color_attributes['Color']
 if mat in ['Cosy_PaleLimestone','Cosy_WhitePaint']:
  for face in me.polygons:
   p=face.center;local=1.0+.04*math.sin(p.x*.81+p.y*.93+p.z*1.21);base=.88 if mat=='Cosy_PaleLimestone' else .94
   under=.89 if face.normal.z<-.25 else 1
   for li in face.loop_indices:
    c=colors.data[li].color;colors.data[li].color=(c[0]*base*local*under,c[1]*base*local*under,c[2]*base*local*under,1)
...
vAll=[];fAll=[]
for ob in bpy.context.scene.objects:
 if ob.type!='MESH' or any(k in ob.data.materials[0].name for k in ['Decal','WindowFilm']):continue
...
for ob in bpy.context.scene.objects:
 ...
 if any(k in mat for k in ['Room','Glass','Curtain','ShopBack','ShopAmber','Decal','WindowFilm']):continue
...
bpy.ops.wm.save_as_mainfile(filepath=str(ROOT/'.dream-loop/architecture-weathered.blend'))
bpy.ops.export_scene.gltf(filepath=str(ROOT/'public/assets/architecture-weathered.glb'),export_format='GLB',export_yup=True,export_apply=True,export_vertex_color='NAME',export_vertex_color_name='Color',export_cameras=False,export_lights=False)
import struct
out=ROOT/'public/assets/architecture-weathered.glb';raw=out.read_bytes();ln=struct.unpack_from('<I',raw,12)[0];doc=json.loads(raw[20:20+ln]);binary=raw[20+ln:]
for mat in doc['materials']:
 if mat['name'] in ['Cosy_DampDecal','Cosy_RunoffDecal']:
  mat['pbrMetallicRoughness']['baseColorFactor']=[1,1,1,.42 if 'Damp' in mat['name'] else .28];mat['alphaMode']='BLEND'
...
(ROOT/'.dream-loop/architecture-weathered-report.json').write_text(json.dumps(report,indent=2));print(json.dumps(report),flush=True)
